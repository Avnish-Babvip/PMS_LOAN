...
import { getAllRiderReferrals } from "../../features/actions/rider";
import FilterSelect from "../../components/FilterSelect";
import { ViewRiderReferralModal } from "../../components/Modal/Rider/ViewRiderReferral";

const RiderReferral = () => {
  const { state } = useLocation();
  const dispatch = useDispatch();
  const [searchParams, setSearchParams] = useSearchParams();
  const { referralData, riderLoading } = useSelector((state) => state.rider);
    ...
    <>
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="font-semibold text-gray-800">All Riders Referrals</h2>
        ...
        {/* PAGINATION */}
        {!riderLoading && hasData && referralData?.meta && (
          <Pagination
            data={referralData?.meta}
            page={page}
            label="riders"
