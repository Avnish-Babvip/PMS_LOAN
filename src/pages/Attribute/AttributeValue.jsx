...
import AddAttributeValueModal from "../../components/Modal/Attribute/AddAttributeValue";
import { EditAttributeValueModal } from "../../components/Modal/Attribute/EditAttributeValue";

const AttributeValue = () => {
  const dispatch = useDispatch();
  const { id } = useParams();
  const { state } = useLocation();
  const { attributeValueData, attributeLoading } = useSelector(
    (state) => state.attribute,
  );
  const data = attributeValueData?.values || [];
  ...
  useEffect(() => {
    if (!openEditModal && !openModal && !openDeleteModal) {
      dispatch(getAllAttributeValues(id));
    }
  ...
  return (
    <>
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h2 className="font-semibold text-gray-800">
            All Attribute Values : {attributeValueData?.name}
          </h2>
      ...
      <AddAttributeValueModal
        ...
        attribute={attributeValueData?.name}
      />
      ...
      <DeleteModal
        ...
        onConfirm={() => {
          dispatch(deleteAttributeValue(selected.id))
            .unwrap()
            .then(() => {
