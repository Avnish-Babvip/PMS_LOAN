...
function TermsAndConditionsForm() {
  ...
  const { termsConditionsData, cmsLoading } = useSelector((state) => state.cms);
  ...
  const handleSingleUpdate = async (item) => {
    ...
    await dispatch(
      updateTermsAndConditions({ payload, id: item?.id }),
    ).unwrap();
...
function CategoryField({ register, setValue, name, value }) {
  ...
  const fetchCategories = async () => {
    try {
      const res = await instance.get(`/admin/termsconditioncategory`, {
        headers: {
  ...
  const handleDeleteCategory = async (id) => {
    try {
      await instance.delete(`/admin/deletetermsconditioncategory/${id}`, {
        headers: {
          Authorization: `Bearer ${loginToken}`,
        },
      });
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      const res = await instance.post(
        `/admin/termsconditioncategorystore`,
        {
          category_name: input,
          status: true,
        },
