...
function PrivacyPolicyForm() {
  ...
  const { privacyPolicyData, cmsLoading } = useSelector((state) => state.cms);
  ...
  const handleSingleUpdate = async (item) => {
    ...
    await dispatch(updatePrivacyPolicy({ payload, id: item?.id })).unwrap();
...
function CategoryField({ register, setValue, name, value }) {
  ...
  const fetchCategories = async () => {
    try {
      const res = await instance.get(`/admin/privacypolicycategory`, {
        headers: {
  ...
  const handleDeleteCategory = async (id) => {
    try {
      await instance.delete(`/admin/deleteprivacypolicycategory/${id}`, {
        headers: {
          Authorization: `Bearer ${loginToken}`,
        },
      });
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      const res = await instance.post(
        `/admin/privacypolicycategorystore`,
        {
          category_name: input,
          status: true,
        },
