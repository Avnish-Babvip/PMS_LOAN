...
import { getAllCoupons } from "../../features/actions/coupon";
import { EditCouponModal } from "../../components/Modal/Coupon/EditCoupon";
import AddCouponModal from "../../components/Modal/Coupon/AddCoupon";

const Coupon = () => {
  const { state } = useLocation();
  const dispatch = useDispatch();
  const [searchParams, setSearchParams] = useSearchParams();
  const { couponData, couponLoading } = useSelector((state) => state.coupon);
  ...
  return (
    <>
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          ...
          <table className="min-w-[900px] w-full text-sm table-fixed">
            <thead className="bg-gray-50 text-gray-500">
              <tr>
                ...
                <th className="text-left px-3 py-3 w-[100px]">Max Discount</th>
                  ...
                  <td colSpan={7} className="py-28">
                    <div className="w-full flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm text-gray-400 mt-1">
                        Try adjusting filters or add a new coupon
                      </p>
                    </div>
                  </td>
                </tr>
        ...
        {/* PAGINATION */}
        {!couponLoading && hasData && couponData?.pagination && (
          <Pagination
            data={couponData.pagination}
            page={page}
            label="coupon"
      ...
      <AddCouponModal
        ...
        onClose={() => {
          setOpenModal(false);
          dispatch(setActiveSubTab("All Coupon Code"));
        }}
      />
      <EditCouponModal
