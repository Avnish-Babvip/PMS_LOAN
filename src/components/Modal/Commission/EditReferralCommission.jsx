...
import { editReferralCommission } from "../../../features/actions/commission";

export const EditReferralCommissionModal = ({
  isOpen,
  onClose,
  commission,
}) => {
  ...
  const { referralLoading } = useSelector((state) => state.commission);
  ...
  const onSubmit = (data) => {
    ...
    dispatch(editReferralCommission({ payload: payload, id: commission?.id }))
      .unwrap()
      .then(() => {
        onClose();
      });
      ...
      >
        ...
        <div className="px-8 pt-8">
          <h2 className="text-center text-black text-xl font-semibold mb-6">
            Edit Referral Commission Value
          </h2>
        </div>
        ...
        <div className="px-8 py-6 border-t border-gray-300">
          <button
            ...
            "
          >
            {referralLoading ? <Spinner /> : "Submit"}
          </button>
        </div>
      </div>
    </form>
